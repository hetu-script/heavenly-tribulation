...
5. `plan/skill_tree/vitality.md:26` 残留旧 id `element_dot_poison`。
...
- `plan/skill_tree/vitality.md:26`：`element_dot_poison` 改为新表述。
...
- 新增 4 个分支标记词条：`spellcraft_branch_guiyuan`（返朴归元）、`spellcraft_branch_tiandao`（天道推演）、`spellcraft_branch_shouyi`（抱真守一）、`spellcraft_branch_wuxing`（五行轮转），description 键 `passive_spellcraft_branch_*_description`。
...
- `ziwei_doushu` 紫微斗数：genre spellcraft、category buff、`retain: true`、`isEphemeral: true`（消耗=打出碎裂）、`coloredCost: {life: 0}`、script `ziwei_doushu`。**确认其不进入随机生成池**（参照 blank_default 的排除方式，必要时在 BattleCard 构造器过滤）。
- `wanfa_guizong` 万法归宗：genre spellcraft、category attack、cardType spell、kind lightning_control、damageType lightning、isUnique、`retain: true`、`coloredCost: {spell: {base: 10, rankIncrement: 0}}`、script `wanfa_guizong`、valueData 提供每层灵气伤害（base 8 起步，数值可调）。
...
- 触发点：`_startTurn` 中 `_prepareStatus(start_turn)`（battle.dart:1245-1251）之后，`currentCharacter` passives 含 `spellcraft_branch_tiandao` → `await scry(3)`（常量 `kScryCardCount = 3` 入 common.dart，绝世装备"+1 张"后续在此加成）。
...
- 新增 external 方法（绑定照抄 `character_binding.dart:95-97` drawCards 模式）：Dart 侧完成"抽 1 张 → 若 `cardType == 'spell' && category == 'attack'` 则将其 `coloredCost` 清零 → `refreshHandCardDescriptions()`（802-823）"，供 `ziwei_doushu` 脚本调用（避免把 CustomGameCard 对象暴露给 hetu）。
...
- **还婴·五气朝元**：新增 `handleWuxingRotation()`，在 `_startTurn` 的 start_turn 注入之后调用——passives 含 `spellcraft_rank_4` 时：先按 `data['wuxingRotation']` 记录移除上轮所授层数，再按 `[waterbend, firebend, earthbend, airbend, lightning_control][turnCount % 5]` 授予对应 `increase_damage_{kind}` ×10 层并记录。常量 `kWuxingRotationKinds`、`kWuxingEnhanceAmount = 10` 入 common.dart；实现时校准 turnCount 初值使首回合为御水术（waterbend）。
- **化神·万法归宗**：战斗开始 `getDeck` 调用点（battle.dart:443 附近）之后——hero passives 含 `spellcraft_rank_5` 时，参照 `_createBlankCard`（318-325）经 hetu `BattleCard(affixId: 'wanfa_guizong')` 造卡、入牌库并洗牌。

### 步骤 10：分支节点效果
- **返朴归元**：战斗开始（同步骤 9 化神注入点）——passives 含 `spellcraft_branch_guiyuan` 时造 `ziwei_doushu` 加入英雄手牌区（翻开）。
- **天道推演**：步骤 6 的 scry 触发。
- **抱真守一 / 五行轮转**：`character.dart` `onUseCard` 末尾（全部词条脚本执行完、cardFlags 就绪后）追加——当前卡经 `isElementCard` 判定为元素牌时：
  - 含 `spellcraft_branch_shouyi`：从对应手牌区随机取一张元素牌，经 hetu `upgradeCard`（card.ht:609，参照 game.dart:1516-1518 的调用方式）升级，hero 侧随后 `refreshHandCardDescriptions()`。
  - 含 `spellcraft_branch_wuxing`：元素键（`elementType ?? damageType`）不在 `turnFlags['usedElements']` 集合中 → 入集合并 `addStatusEffect('energy_positive_spell', amount: 1)`（turnFlags 每回合自动清空，天然实现"每回合每种一次"）。

### 步骤 11：卡牌脚本（card_script.ht）
- `ziwei_doushu(self, opponent, affix, mainAffix)`：调用步骤 7 的新 external。
- `wanfa_guizong(self, opponent, affix, mainAffix)`：读 `cardFlags['paidCost']['energy_positive_spell']`（已付 10 点，refund_cost 同款先例）+ `hasStatusEffect('energy_positive_spell')` 剩余层数 → `removeStatusEffect(force: true)` 清空 → 总耗费 × `affix.value[0]` 为 baseValue，damageDetails（damageType: 'lightning'，cardType: 'spell'）→ `opponent.takeDamage(...)`（参照 attack 脚本 21-29 与 convert_vigor_all 111-117 的写法）。
