...
1. **三张 exhaust_mana 牌拆分**：punch / sword / flying_sword 各拆分为火/冰/雷三张
   （id 加 `_fire` / `_ice` / `_lightning` 后缀）；剧情脚本 starter 引用统一改用 `_fire` 版本。
2. **consume 类额外词条整体移除**：5 条"消耗气换额外效果"的额外词条全部删除，
   该机制以后只出现在主词条。
3. **overflowed 词条改随机元素版**：`overflowed_mana_deal_elemental_damage` 改为
   `overflowed_mana_deal_random_element_damage`（溢出时随机造成火/冰/雷伤害），
   且本阶段首次接上实现（此前为死数据）。
4. **降抗词条 = weakness 永久状态**：以降低对手火/冰/雷/毒抗的弱点（负抗性）形式实现，持续本场战斗。
5. **冰缓最终机制**：不扣血；持有者回合结束时所有层一次性分别随机转化为缓慢或迟钝
   （回合结束触发，与幻觉的回合开始形成节奏区分）。
6. **冰/毒新卡各 1 张**：占位美术先行验证流程，正式插画后补。
7. **淬毒装备系统不在本阶段落地**：仅落地毒攻击卡牌与中毒 DOT。
8. **存档不兼容声明**：旧存档宣布不兼容，不做迁移。
...
| 御风术 airbend         | elemental  | **physical**  | 风刃如刀；仍是法术牌，吃灵气加成；可暴击 |
...
4. **元素持续伤害（DOT）**：统一规则 + 四种参数（设计定案，参数为初值）。
   ...
   | 雷   | `element_dot_lightning` | 感电 | 1~10                      | 随机            | 每回合随机消耗层数，每层随机造成伤害，每次伤害单独跳字。                            |
   | 冰   | `element_dot_ice`       | 冰缓 | 0（不扣血）               | 不衰减          | 持有者回合结束时所有层一次性分别随机转化为缓慢或迟钝；回合结束触发，与幻觉的回合开始形成节奏区分，绵长干扰 |
