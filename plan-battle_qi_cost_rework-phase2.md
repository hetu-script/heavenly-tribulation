...
1. **移除 role 字段** ✅ status_effect.json5 中 4 处 `role: "cost"` + 1 处 `role: "wildcard"` 及头部 3 行 role 说明全部删除（保留 isResource 说明行，它与 role 无关）。
2. **costChi → costColored** ✅ game.dart 校验方法改名 `_validateBattleCardCostColored`，字段引用、注释、警告文案同步。
3. **范围收窄** ✅ 未触碰 passive_skills.json5、passives.json5、lib/scene/card_library/、lib/widgets/character/stats.dart。

## Phase 2 任务

**2.1 费用数据落地** ✅
- game.dart 新增 `deriveBattleCardCost`（game.dart:1425-1461）：显式 costColored 优先（cost = rank+1−Σ）；缺省推导有色 = ⌈(rank+1)/2⌉、无色 = ⌊(rank+1)/2⌋、无流派全无色、多色对半拆且余数给列表首位（avatar 奇数时多点给 unarmed）。`createBattleCard` 写入 `cardData['cost']`/`['costColored']` 且卡面 `cost:` 用推导出的无色费用（game.dart:1481-1484, 1537）。
- card.ht BattleCard 构造器在 rank 定稿后计算 `this.cost`/`this.costColored`（card.ht:136-165），逻辑与 Dart 侧一致（注释互相指引）。
...
**2.4 过渡期卡面文本** ✅ getBattleCardDescription 在描述区附加"另需：剑气×2、煞气×1"行（game.dart:1389-1406），颜色名复用 `status_energy_positive_*` 键，多色用"、"分隔。新增本地化键 `battlecard_costColored_hint`（"另需：{0}"）与 `enumeration_separator`（"、"）。

**2.5 验收** ✅ dart analyze 全仓仅 3 条预存 pubspec 警告；`python build.py` 两模组编译成功（反查 main.mod 确认 10 个 exhaust 函数清零、5 个新函数已编入）；数据自检脚本程序化验证 80 张卡：**DATA CHECK OK**（52 张显式 costColored 不变量全过、无 exhaust 脚本/关键字残留、valueData 长度与脚本匹配）。
